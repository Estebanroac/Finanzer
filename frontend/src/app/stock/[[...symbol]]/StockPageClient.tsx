...
function AnalysisContent({ data, symbol }: { data: StockAnalysis; symbol: string }) {
  ...
  // 52-week position (0-100%) + qué tan lejos del máximo (lectura rápida de "cara")
  let range52pct = 50;
    ...
    <div className="min-h-screen relative">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-16">
        ...
        <header className="mb-6 fade-up">
          ...
          <p className="text-sm text-zinc-500 truncate mb-4 sm:mb-5">
            {data.profile.sector} · {data.profile.industry}
            {data.profile.country && data.profile.country !== "Unknown" && ` · ${data.profile.country}`}
          </p>

          {/* Hero de precio: precio prominente + rango 52 semanas integrado, para
              leer de un vistazo el precio y qué tan cara está la acción. */}
          <div className="rounded-2xl border border-white/[0.07] bg-[#0a0a0d]/85 px-5 sm:px-7 py-5 sm:py-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <a
                ...
                className="shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-medium text-zinc-300 bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] active:scale-95 transition-all"
              >
            ...
            {hi52 && lo52 && (
              <div className="mt-6 pt-5 border-t border-white/[0.06]">
                ...
                <div className="flex justify-between text-[11px] tabular-nums text-zinc-500 mt-2">
                  <span>{formatPrice(lo52)}</span>
                  <span>{formatPrice(hi52)}</span>
                </div>
              </div>
            )}
          </div>
